...
class FileUtils {
    ...
    static detectFilePath(formats) {
        ...
        try {
            ...
            if (!detectedPath && formats.includes('text/uri-list')) {
                const uriList = electron_1.clipboard.read('text/uri-list');
                // Silenced log
                const lines = uriList.split(/[\r\n]+/);
                const fileLine = lines.find(line => line.trim().startsWith('file://'));
                if (fileLine) {
                    try {
                        detectedPath = (0, url_1.fileURLToPath)(fileLine.trim());
                        // Silenced log
                    }
                    catch (e) {
                        // Silenced log
            ...
            if (!detectedPath) {
                ...
                let text = electron_1.clipboard.readText().trim();
                // Common issue: "file:///Users/..." copy-pasted as text
                if (text.startsWith('file://')) {
                    try {
                        text = (0, url_1.fileURLToPath)(text);
                    }
                    catch (e) { }
                }
                ...
                if (!detectedPath && text.length > 0 && !text.includes('/') && !text.includes('\\')) {
                    try {
                        const downloadsPath = path_1.default.join(electron_1.app.getPath('downloads'), text);
                        const desktopPath = path_1.default.join(electron_1.app.getPath('desktop'), text);
                        ...
                        else if (fs_1.default.existsSync(desktopPath)) {
                ...
                if (!detectedPath && text.startsWith('/')) {
                    if (fs_1.default.existsSync(text)) {
                        try {
                            const stats = fs_1.default.statSync(text);
                            if (stats.isFile()) {
                                detectedPath = text;
     ...
     */
    static async generateFilePreview(filePath) {
        let preview = '';
        ...
        try {
            ...
            try {
                const thumbnail = await electron_1.nativeImage.createThumbnailFromPath(filePath, { width: 600, height: 600 });
